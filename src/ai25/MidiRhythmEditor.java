...
public class MidiRhythmEditor extends JFrame {
    ...
    private void initMidiEngine() {
        try {
            ...
            sequencer.open();
            midiReceiver = sequencer.getReceiver(); // MIDI 출력을 위한 통로 확보
    ...
    private void togglePlayback() {
        ...
        if (sequencer.isRunning()) {
            ...
            if (uiSyncTimer != null) uiSyncTimer.stop();
            uiSyncTimer = new Timer(10, e -> syncTableSmooth()); // 정밀도를 위해 10ms로 설정
            uiSyncTimer.start();
    ...
    private void playNote(int pitch) {
        try {
            ...
            // 200ms 후 소리 끄기 (자연스러운 연주를 위해)
            Timer offTimer = new Timer(200, e -> {
                try {
